...
namespace KTH.API.Controller
{
    ...
    [ApiController]
    public class TransLrController : ControllerBase
    {
        private readonly ITransLrService _service;

        public TransLrController(ITransLrService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(string caseId)
        {
            ...
            try
            {
                var Outbound = await _service.GetTransLrByCaseId(caseId);
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] AddTransLrRequest param)
        {
            ResponseModel resp = new ResponseModel();

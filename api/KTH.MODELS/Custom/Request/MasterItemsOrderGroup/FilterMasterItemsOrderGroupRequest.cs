...
{
    public class FilterMasterItemsOrderGroupRequest : FilterModel
    {
        public FilterMasterItemsOrderGroupRequest()
        {
        ...
        public static List<MasterItemsOrderGroupListResponseData> ApplySorting(List<MasterItemsOrderGroupListResponseData> queryable, string sortName, string sortType)
        {
            if (!string.IsNullOrEmpty(sortName))
            {
                PropertyInfo propertyInfo = typeof(MasterItemsOrderGroupListResponseData).GetProperty(sortName, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);

                if (propertyInfo != null)
                {
                    var parameter = Expression.Parameter(typeof(MasterItemsOrderGroupListResponseData), "x");
                    var property = Expression.Property(parameter, propertyInfo);
                    var lambda = Expression.Lambda<Func<MasterItemsOrderGroupListResponseData, object>>(

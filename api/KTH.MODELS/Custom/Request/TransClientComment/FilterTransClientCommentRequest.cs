...
{
	public class FilterTransClientCommentRequest : FilterModel
	{
		public FilterTransClientCommentRequest()
		{
        ...
        public static List<TransClientCommentListResponseData> ApplySorting(List<TransClientCommentListResponseData> queryable, string sortName, string sortType)
        {
            if (!string.IsNullOrEmpty(sortName))
            {
                PropertyInfo propertyInfo = typeof(TransClientCommentListResponseData).GetProperty(sortName, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);

                if (propertyInfo != null)
                {
                    var parameter = Expression.Parameter(typeof(TransClientCommentListResponseData), "x");
                    var property = Expression.Property(parameter, propertyInfo);
                    var lambda = Expression.Lambda<Func<TransClientCommentListResponseData, object>>(

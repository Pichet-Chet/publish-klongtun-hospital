...
{
	public class FilterTransConsultCommentRequest : FilterModel
	{
        public FilterTransConsultCommentRequest()
        {
        ...
        public static List<TransConsultCommentListResponseData> ApplySorting(List<TransConsultCommentListResponseData> queryable, string sortName, string sortType)
        {
            if (!string.IsNullOrEmpty(sortName))
            {
                PropertyInfo propertyInfo = typeof(TransConsultCommentListResponseData).GetProperty(sortName, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);

                if (propertyInfo != null)
                {
                    var parameter = Expression.Parameter(typeof(TransConsultCommentListResponseData), "x");
                    var property = Expression.Property(parameter, propertyInfo);
                    var lambda = Expression.Lambda<Func<TransConsultCommentListResponseData, object>>(
